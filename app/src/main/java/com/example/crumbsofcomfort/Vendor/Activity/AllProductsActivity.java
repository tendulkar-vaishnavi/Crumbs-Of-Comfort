...
import com.example.crumbsofcomfort.databinding.ActivityAllProductsBinding;
...
public class AllProductsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding.recyclerAllProducts.setLayoutManager(new LinearLayoutManager(this,LinearLayoutManager.VERTICAL,false));
        binding.recyclerAllProducts.setAdapter(productListAdapter);

        vendorId = FirebaseAuth.getInstance().getCurrentUser().getUid();
        fetchShopIdAndLoadProducts();
    }

    private void fetchShopIdAndLoadProducts() {
        ...
        userRef.child("shopId").addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.exists()) {
                    ...
                    Toast.makeText(AllProductsActivity.this, "Shop ID not found", Toast.LENGTH_SHORT).show();
                }
            }

            @Override
            public void onCancelled(@NonNull DatabaseError error) {
                Log.e(TAG, "Shop ID fetch cancelled: " + error.getMessage());
            }
        });
    }
